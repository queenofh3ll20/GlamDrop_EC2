...
addSafeListener('register-form', 'submit', async (e) => {
  ...
  try {
    ...
    alert('Registrazione completata! Esegui ora il login.');
...
async function renderSalonsGrid() {
  ...
  // Deduplicate currentSalonsList strictly in frontend as fail-safe
  const seenSalons = new Set();
  const uniqueSalonsList = [];
  currentSalonsList.forEach(s => {
    const key = `${(s.name || '').toLowerCase().trim()}_${(s.street || '').toLowerCase().trim()}`;
    if (!seenSalons.has(key) && !key.includes('test') && !key.includes('scadenza')) {
      seenSalons.add(key);
...
addSafeListener('add-drop-form', 'submit', async (e) => {
  ...
  const serviceId = document.getElementById('drop-service-select').value;
  const employeeId = document.getElementById('drop-employee-select').value;
  const timeInput = document.getElementById('drop-time').value; // 'YYYY-MM-DDTHH:MM'
  const discountedPrice = parseFloat(document.getElementById('drop-price').value);

  if (!serviceId || !employeeId || !timeInput || !discountedPrice) {
    alert('Tutti i campi sono obbligatori');
    return;
  }
  ...
  try {
    ...
    if (res.ok) {
      alert('Drop Last-Minute pubblicato con successo!');
